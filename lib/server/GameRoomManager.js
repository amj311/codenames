...
class GameRoomManager {
    ...
    handleLostSocket(socket) {
        ...
        let captainTeam = GameHelpers.getCaptainsTeam(lostConn.userData,this.game.teams);
        if (captainTeam) {
            lostConn.setMeta('wasCaptainOfTeam',captainTeam);
        }
    ...
    handleLostUserRecord(record) {
        ...
        if (record.hasMeta("wasCaptainOfTeam")) {
            this.game.setTeamCaptain(false,record.getMeta("wasCaptainOfTeam").id,userData);
    ...
    handleReturningPlayer(newSocket,oldSockId,cb) {
        ...
        if (oldConnPair.hasMeta("wasCaptainOfTeam")) {
            let oldTeamId = oldConnPair.getMeta("wasCaptainOfTeam").id;
